...
class ForestCoverData():
  ...
  def remove_wilderness_area(self, data):
  ...
  ##########Binary Classifiers########################################
  def train_on_partitions(X=None, y=None):
    ...
    for v in ct_vals:
      ...
      classifiers[-1].fit(X, target)
      predictions.append(classifiers[-1].predict(test))
  ...
  def classify_remaining(no, multi, solid):
    ...
    s, knn = knn_cross_validate(X_lim, y, keywords={"n_neighbors":3, "weights":"distance", "p":1})
  ...
  def plot_records(xind, yind, records, wilderness=None, color='c.'):
    if wilderness != None:
  ...
  def cross_validate(self, classifier_type, args=(), keywords={}, test_size=0.3, X=None, y=None, random_state=5, just_score=False):
  ...
  def rf_cross_validate(self, n=100, X=None, y=None):
    ...
    if X == None or y == None:
      X = self.X_no_id
      y = self.y
    return self.cross_validate(RandomForestClassifier, (n,), keywords={'random_state':5}, test_size=.2, X=X, y=y)

  def knn_cross_validate(self, X=None, y=None, keywords={"n_neighbors":5, "weights":"distance",'p':1}, test_size=.3, random_state=5, just_score=False):
    if X == None or y == None:
      X = self.X_no_id
      y = self.y
